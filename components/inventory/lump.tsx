...
export default function LumpComponents() {
  ...
  useEffect(() => {
    const fetchLumps = async () => {
      const { data, error } = await supabase.from("lumped").select("*");
      if (error) {
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {visibleLumps.map((lump) => (
          ...
          >
            ...
            <h2 className="text-lg font-semibold text-white">{lump.name}</h2>
          </Card>
        ))}
      </div>

      {/* Show More / Show Less */}
      {lumps.length > visibleCount && (
